...
func isUserAccountExist(c *gin.Context, userAccount string) bool {
	affected := database.DB.Take(&model.User{}, "user_account = ?", userAccount).RowsAffected
	if affected == 1 {
...
func bindContextJson(c *gin.Context, data interface{}) bool {
	...
	if err != nil {
		fmt.Println("[api user bindContextJson err] c.ShouldBindJSON : ", err.Error())
...
func updateUserInfo(c *gin.Context, user model.UserInfo) bool {
	//零值不更新
	affected := database.DB.Take(&model.User{}, user.ID).Updates(model.User{
...
func changePassword(c *gin.Context, user model.UserChangePassword) bool {
	// 判断密码和确认密码是否一致
