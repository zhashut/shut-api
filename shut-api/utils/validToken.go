...
func ValidToken(c *gin.Context) {
	...
	// 从redis中进行查询
	result, err := database.Client.HMGet(database.Ctx, tokenKey, "client_ip", "id").Result()
	// 没查到就会报err
	if err != nil {
	...
	affected := database.DB.Select("role", "user_account").
		Take(&model.User{}, "id = ?", result[1]).Scan(&userData).RowsAffected
	if affected == 0 {
		c.Set("userRole", RoleUndefined)
		return
	}
